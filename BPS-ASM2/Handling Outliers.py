...
threshold_lower = -8  # Lower threshold for outlier value

outliers = []
for column in outlier_columns:
    z_scores = np.abs((df[column] - df[column].mean()) / df[column].std())  # Calculate z-score for each value
    outliers.extend(df[column][z_scores > threshold_upper].index.tolist())  # Add indexes of outliers that exceed the upper threshold
